"""New-generation extended Kalman attitude estimators.

The two filters share one explicit lifecycle and differ only in attitude-error
coordinates:

* :class:`EKF` stores a normalized four-component quaternion covariance.
* :class:`MEKF` stores a three-component right attitude-error covariance.

Both compose the filter-neutral ``State``, ``Covariance``, process-model, and
...
__all__ = ["AttitudeEstimator", "EKF", "MEKF"]
