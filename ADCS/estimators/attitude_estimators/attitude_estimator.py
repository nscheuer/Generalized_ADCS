...
class AttitudeEstimator:
    ...
    def __init__(
        ...
        *,
        ...
        # Keep the caller's per-step Q separate from the total Q exposed on a
        # predicted state.  Otherwise the model-generated part would be added
        # again on the following prediction.
    ...
    def _add_state_process_noise(self, process_noise: np.ndarray) -> np.ndarray:
        """Add the state's discrete per-step process covariance.

        ``discretize_process_noise`` returns the covariance generated by the
        continuous model. The configured
        ``EstimatorState.process_noise`` (the legacy ``int_cov`` field) is an
        independent, already-discrete covariance and therefore must be added
        after discretization.
        """
        model_noise = np.asarray(process_noise, dtype=float)
        state_noise = self._configured_process_noise.as_matrix()
        if model_noise.shape != state_noise.shape:
            raise ValueError(
                "discretized process noise and state process noise must have "
                f"matching shapes, got {model_noise.shape} and {state_noise.shape}"
            )
        combined = model_noise + state_noise
