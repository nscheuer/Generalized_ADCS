...
class Plan_and_Track_LQR(Controller):
    def __init__(self, est_sat: EstimatedSatellite, planner_settings: PlannerSettings) -> None:
        ...
        # High weight prevents saturation given low max torque (~0.005 Nm)
    ...
    def find_u(self, x_hat: np.ndarray, sens: np.ndarray, est_sat: EstimatedSatellite, os_hat: Orbital_State, goal_vector_eci: np.ndarray | None = None, w_ref: np.ndarray | None = None) -> np.ndarray:
        ...
        # Safety check for valid quaternion reference
    ...
    def calculate_trajectory(self, t_start: float, duration: float, x_0: np.ndarray, os_0: Orbital_State, goals: GoalList, verbose: bool = False) -> Trajectory:
        ...
        # Append zero gain for the final time step to match dimensions
    ...
    def _altro_solve(self, X_init, U_init, dt, os_list, goals, verbose=False):
        ...
        # Calculate initial cost using the dynamic goals
        current_cost = self._trajectory_cost(X, U, goals, os_list)
    ...
    def _backward_pass(self, X: np.ndarray, U: np.ndarray, dt: float, os_list: List[Orbital_State], goals: GoalList, reg: float):
        ...
        # --- Terminal Cost Setup ---
        # Dynamic Goal: Get target quaternion for the final time step
        x_ref_N = self._get_reference_state(os_list[-1].J2000, os_list[-1], goals)

        dx = X[-1] - x_ref_N
        if self.nx >= 7 and np.dot(X[-1, 3:7], x_ref_N[3:7]) < 0:
            dx[3:7] = X[-1, 3:7] + x_ref_N[3:7]
        ...
        for k in range(N-2, -1, -1):
            ...
            # --- Dynamic Goal for step k ---
            x_ref_k = self._get_reference_state(os_k.J2000, os_k, goals)
            ...
            if k == N-2:
                print(f"DEBUG B_MAT: Max Val={np.max(np.abs(B_c_T))}")
                if np.max(np.abs(B_c_T)) < 1e-9:
                    print("!!! WARNING: ACTUATOR JACOBIAN IS ZERO !!!")
            
            # 2nd Order Discretization (better than Euler)
            A_sq = A_c @ A_c
            A_k = np.eye(self.nx) + A_c * dt + 0.5 * A_sq * (dt**2)
            B_k = B_c_T.T * dt + 0.5 * (A_c @ B_c_T.T) * (dt**2)
            
            # Calculate local cost derivatives relative to Reference
            dx_k = x_k - x_ref_k
            if self.nx >= 7 and np.dot(x_k[3:7], x_ref_k[3:7]) < 0:
                dx_k[3:7] = x_k[3:7] + x_ref_k[3:7]
            ...
            if k == N-2:
                print(f"DEBUG GRADIENT: |Vx|={np.linalg.norm(Vx):.4e}, |Qu|={np.linalg.norm(Qu):.4e}")
    ...
    def _forward_pass(self, X_old, U_old, Ks, ds, dt, os_list, goals, current_cost):
        ...
        for alpha in alphas:
            ...
            # Pass os_list so cost function can look up dynamic reference
            cost = self._trajectory_cost(X_new, U_new, goals, os_list)
    ...
    def _trajectory_cost(self, X: np.ndarray, U: np.ndarray, goals: GoalList, os_list: List[Orbital_State]) -> float:
        J = 0.0
        N = X.shape[0]

        for k in range(N-1):
            # Dynamic Target Lookup
            x_ref = self._get_reference_state(os_list[k].J2000, os_list[k], goals)

            dx = X[k] - x_ref
            if self.nx >= 7 and np.dot(X[k, 3:7], x_ref[3:7]) < 0:
                dx[3:7] = X[k, 3:7] + x_ref[3:7]
    ...
    def _get_reference_state(self, t: float, os: Orbital_State, goals: GoalList) -> np.ndarray:
        """
        Calculates the full state vector reference (x_ref) based on the goals.
        Computes the 'Shortest Arc' quaternion to rotate ECI Target -> Body Boresight.
        """
        ...
        # Wheels (indices 7+) default to 0 momentum reference (already zero from np.zeros)
