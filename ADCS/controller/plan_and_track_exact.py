"""
Plan and Track Exact Controller for spacecraft attitude control.

This module implements a trajectory-following controller that uses the ALTRO
trajectory planner to compute optimal trajectories and executes them open-loop
without feedback tracking.
"""
...
class Plan_and_Track_Exact(PlanAndTrackBase):
    """
    Trajectory-following controller using ALTRO planning with open-loop execution.

    This controller computes optimal trajectories using the ALTRO (Augmented
    Lagrangian TRajectory Optimizer) and executes them open-loop without
    feedback tracking - useful for testing trajectory quality.
    ...
    def __init__(self, est_sat: EstimatedSatellite, planner_settings: PlannerSettings) -> None:
        """
        Initialize the Plan and Track Exact controller.
    ...
    def find_u(
        ...
        Args:
            x_hat: Estimated state vector (unused - open loop)
