...
.. note::
   ...
   The ``ADCS/pipeline/`` tree below is landing in ``main`` with the adapter
   PR. The legacy controllers (``ADCS/controller/mtq_w_rw_LP.py`` and
   friends) are in the released package today and stay as the validated
   baselines.
