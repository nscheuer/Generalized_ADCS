...
from ADCS.helpers.plotting_mc.plot_controller_mc import plot_target_tracking_mc, plot_convergence_histogram_mc,plot_h_tracking_mc
...
def run_single_sim(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 6. Controller
        controller = MTQ_w_RW_QPC(est_sat=real_sat, p_gain=0.00005, d_gain=0.001, c_gain=0.001, h_target=np.array([0.004, 0.0, 0.0]))
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data("3MTQ+1RW_QPC_mc_36", full_results, out_dir=OUTPUT_DIR)
        
        plot_target_tracking_mc(full_results=full_results, title="3 MTQ + 1 RW QPC MC:36")
        plot_h_tracking_mc(full_results=full_results, title="3 MTQ + 1 RW QPC MC:36")

        plot_convergence_histogram_mc(full_results=full_results, title="3 MTQ + 1 RW QPC MC:36 $|Mu - \\tau)|^2$")
