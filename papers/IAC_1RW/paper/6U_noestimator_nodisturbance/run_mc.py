"""Run any no-estimator / no-disturbance 6U MC configuration (10 runs by default)."""
...
def main() -> None:
    ...
    run_monte_carlo(number_rw=args.architecture, allocator=args.allocator,
                    label=f"3 MTQ + {args.architecture} RW (no estimator, {args.goal_type}, {args.allocator.upper()})",
                    goal_type=args.goal_type, use_estimator=False,
                    campaign_prefix="6u_noestimator_nodisturbance", output_dir=Path(__file__).resolve().parent / "outputs",
