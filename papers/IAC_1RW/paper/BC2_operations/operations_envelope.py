...
def plot_pointing_time(methods: list[MethodResults]) -> Path:
    ...
    for bound_deg, offset, hatch in ((5.0, -width / 2.0, ""), (1.0, width / 2.0, "//")):
        ...
        for index, bar in enumerate(bars):
            ...
            if index == 2 and bound_deg == 5.0:
                x_text -= 0.22
            ax.text(x_text,
                    means[index] + sigmas[index] + 2.5,
                    f"{means[index]:.1f}%", ha="center", va="bottom", fontsize=6.0)

    ax.set_xticks(positions, ["Reactive\n75/25", "Co-allocation\n$\\gamma=0.5$", "Planner"])
    ax.tick_params(axis="x", labelsize=7.0)
    ax.set_ylabel("Time within pointing bound [% of run]")
    ax.set_ylim(0.0, 108.0)
    ax.set_title("6U time within pointing bounds\n(mean ± 1σ across Monte Carlo runs)")
    ax.grid(True, axis="y")
    ax.legend(title="Pointing bound", loc="upper right", fontsize=6.2, title_fontsize=6.5, frameon=True)
    fig.tight_layout()
