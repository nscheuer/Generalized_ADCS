...
RW_TORQUE_MAX = AUTHORITY_SCALE * 1.2e-5
MTQ_DIPOLE_MAX = AUTHORITY_SCALE * (0.20 / 3.0)
...
def rw_cube():
    vertices = np.array(np.meshgrid(*[[-RW_TORQUE_MAX, RW_TORQUE_MAX]] * 3)).T.reshape(-1, 3)
    faces = ((0, 1, 3, 2), (4, 5, 7, 6), (0, 1, 5, 4),
             (2, 3, 7, 6), (0, 2, 6, 4), (1, 3, 7, 5))
    return vertices, [[vertices[i] for i in face] for face in faces]


def minkowski_points(b_field):
    """Return all vertex sums of the RW cube and MTQ polygon."""
    cube, _ = rw_cube()
    return (cube[:, None, :] + mtq_plane(b_field)[None, :, :]).reshape(-1, 3)
...
def plot_cube_and_plane():
    ...
    save(figure, "01_rw_cube_mtq_plane")
...
def plot_progression():
    figure, ax = setup("3 MTQ + 3 RW: authority progression")
    ...
    save(figure, "03_minkowski_progression")
