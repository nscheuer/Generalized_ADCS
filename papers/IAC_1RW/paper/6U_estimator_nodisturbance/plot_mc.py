...
PERCENTILE_TITLE = "6U attitude convergence (median, 10--90 percentile)"
PERCENTILE_YLABEL = "Median angle error [deg]"
_RESULTS_CACHE = {}
_TRACE_CACHE = {}
...
def _load_results(path: Path) -> ADCS.SimulationResults:
    """Load each Monte Carlo dataset once for all convergence plots."""
    key = str(path)
...
def _aligned_values(results: ADCS.SimulationResults):
    """Return the common time grid and cached per-run angle traces."""
    key = id(results)
    if key not in _TRACE_CACHE:
        traces = [_angle_error_deg(run) for run in results.runs]
        time = traces[0][0]
        values = np.vstack([np.interp(time, run_time, angle)
                            for run_time, angle in traces])
        _TRACE_CACHE[key] = time, values
...
def _summary(results: ADCS.SimulationResults) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    time, values = _aligned_values(results)
    return time, np.mean(values, axis=0), np.std(values, axis=0)


def _percentile_summary(results: ADCS.SimulationResults) -> tuple[
        np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Return time, median, and pointwise 10th/90th percentile convergence."""
    time, values = _aligned_values(results)
    p10, median, p90 = np.percentile(values, (10, 50, 90), axis=0)
    return time, median, p10, p90
...
def _plot_median_percentile(output_dir: Path) -> None:
    """Plot median convergence with a pointwise 10th–90th percentile band."""
    fig, ax = plt.subplots(figsize=(3.45, 2.45))
    colors = ("#0072B2", "#D55E00", "#009E73")
    ...
    for (number_rw, allocator, label), color in zip(CONFIGURATIONS, colors):
        result_path = _latest_sim(output_dir, number_rw, allocator)
        results = _load_results(result_path)
        time, median, p10, p90 = _percentile_summary(results)
        time_min = time / 60.0
        ax.plot(time_min, median, color=color, label=label)
        ax.fill_between(time_min, p10, p90, color=color, alpha=0.18, linewidth=0.0)

    ax.set_xlabel("Time [min]")
    ax.set_ylabel(PERCENTILE_YLABEL)
    ax.set_title(PERCENTILE_TITLE)
    ax.grid(True)
    ax.set_ylim(bottom=0.0)
    ax.legend(title="Configuration", ncol=3, loc="upper right", frameon=True)
    fig.tight_layout()
    path = output_dir / f"{CAMPAIGN_PREFIX}_angle_convergence_mc_median_p10_p90.png"
    fig.savefig(path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {path}")
...
def main() -> None:
    ...
    plt.close(fig)
    print(f"Saved {path}")
    _plot_median_percentile(output_dir)
