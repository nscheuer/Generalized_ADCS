"""Run any no-estimator / disturbance 6U MC configuration (10 runs by default)."""
...
def main() -> None:
    ...
    run_monte_carlo(number_rw=args.architecture, allocator=args.allocator,
                    label=f"3 MTQ + {args.architecture} RW (no estimator, disturbed, {args.goal_type}, {args.allocator.upper()})",
                    goal_type=args.goal_type, use_estimator=False, disturbances=True,
                    campaign_prefix="6u_noestimator_disturbance", output_dir=Path(__file__).resolve().parent / "outputs",
