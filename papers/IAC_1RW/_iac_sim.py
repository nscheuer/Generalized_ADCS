...
# 300 s is >5x a typical 3+1 solve (~50 s) and far under the observed pathological
# case (14.5 h CPU): a solve needing more than 5x typical is operationally
...
def _plan_in_child(seconds, fn, *a, **kw):
    ...
    was inert exactly when it was needed (observed: 2 of 100 solves ran 14.5 h CPU
    each with every alarm armed and none fired, stalling the pool).
...
def simulate(config: Dict[str, Any],
    ...
    try:
        ...
        # censored). The threshold-sensitivity check needs the DISTRIBUTION:
