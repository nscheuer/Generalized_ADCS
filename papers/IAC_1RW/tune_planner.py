"""Planner warm-start + cost-weight tuning study.

One 'reasonable but slightly difficult' candidate: seed 49 -- a PD-diverged frontier
draw the planner rescues but holds imprecisely (2.80 deg final in the 3+1 cell).
...
def run_one_pd(kp_mult, seed, task="full", inc_deg=None, save_dir=None,
    ...
    inclination override and per-trial persistence."""
...
def campaign_running():
    """One job at a time on the machine: stacked launches produce worker-aging
    artifacts and memory-pressure kills. Refuse if the campaign generator is active OR
...
def _run_mode(mode):
    ...
    if mode == "--spread":
        ...
        # CROSSOVER: kill-seeds FIRST (fresh workers), spread
    ...
    if mode == "--pdcheck":
        ...
        return 0
    if mode == "--validate":
        # Validation of the per-task candidates:
        ...
        return 0
    if mode == "--wave":
        # The rerun wave for the frozen per-task configurations.
        ...
        # Cell E -- the clean baseline-weights planner-full cell, run ahead of the PD
        # cells. SEED-PAIRED with cell 2 by construction: make_config(s,
        # seed=s) for s in 0..99, identical draws -- the paired deltas measure what
        # aging actually cost. Stock controller, cold, exactly
        ...
        return 0
    if mode == "--bridge":
        # Intermediate configuration: angle ~3e1 flat.
        import glob as _glob
        import multiprocessing as mp
    ...
    if mode == "--qpcell":
        ...
        return 0
    if mode == "--lowinc":
        # Low-inclination spot-check: PD 3+1 reduced at inc=15 deg, n=30,
        # campaign gains, per-trial persistence (dwell direction, divergence
        # fraction, screen false-alarm rate).
        import multiprocessing as mp
