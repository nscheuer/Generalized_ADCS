...
THREE_RW_AUTHORITY_SCALE = 0.75
MAGNETIC_FIELD_COLOR = "#009E73"
...
def mtq_plane(b_field, mtq_dipole_max=MTQ_DIPOLE_MAX):
...
def rw_vertices(number_rw, rw_torque_max=RW_TORQUE_MAX):
    if number_rw == 0:
        return np.zeros((1, 3))
    if number_rw == 1:
...
def combined_points(b_field, number_rw, mtq_dipole_max=MTQ_DIPOLE_MAX,
                    rw_torque_max=RW_TORQUE_MAX):
    plane = mtq_plane(b_field, mtq_dipole_max)
    return (plane[:, None, :] + rw_vertices(number_rw, rw_torque_max)[None, :, :]).reshape(-1, 3)
...
def add_rw(ax, number_rw, rw_torque_max=RW_TORQUE_MAX):
    if number_rw == 1:
        line = rw_vertices(1, rw_torque_max)
        ax.plot(*line.T, color=RED, linewidth=1.2)
    elif number_rw == 3:
        vertices = rw_vertices(3, rw_torque_max)
...
def add_magnetic_field(ax, b_field):
    """Show the magnetic-field direction from the origin."""
    direction = unit(b_field) * 1.25e-5
    ax.quiver(0.0, 0.0, 0.0, *direction, color=MAGNETIC_FIELD_COLOR,
              linewidth=1.3, arrow_length_ratio=0.14)
    ax.text(*(1.08 * direction), r"$B$", color=MAGNETIC_FIELD_COLOR,
             fontsize=7.0)


def add_combined(ax, b_field, number_rw, mtq_dipole_max=MTQ_DIPOLE_MAX,
                 rw_torque_max=RW_TORQUE_MAX, color=BLUE, alpha=0.20):
    if number_rw == 0:
        add_polygon(ax, mtq_plane(b_field, mtq_dipole_max), color, alpha)
        return
    points = combined_points(b_field, number_rw, mtq_dipole_max, rw_torque_max)
...
def plot_separate(name, number_rw, authority_scale=1.0):
    figure, ax = setup(f"{name}: separate actuator authority")
    add_polygon(ax, mtq_plane(B_NOW, authority_scale * MTQ_DIPOLE_MAX))
    add_rw(ax, number_rw, authority_scale * RW_TORQUE_MAX)
    add_magnetic_field(ax, B_NOW)
    handles, labels = separate_legend(number_rw)
    handles.append(Line2D([0], [0], color=MAGNETIC_FIELD_COLOR, linewidth=1.3))
    labels.append(r"$B$")
...
def plot_combined(name, number_rw, b_field, stem, field_label, authority_scale=1.0):
    figure, ax = setup(f"{name}: combined authority ({field_label})")
    add_combined(ax, b_field, number_rw,
                 authority_scale * MTQ_DIPOLE_MAX, authority_scale * RW_TORQUE_MAX)
    add_magnetic_field(ax, b_field)
    ax.legend([Patch(facecolor=to_rgba(BLUE, 0.20), edgecolor=BLUE),
               Line2D([0], [0], color=MAGNETIC_FIELD_COLOR, linewidth=1.3)],
              ["MTQ plane $\u2295$ RW authority", r"$B$"], loc="upper left",
...
def make_plot():
    configurations = (("3+0", 0, 1.0), ("3+1", 1, 1.0),
                      ("3+3", 3, THREE_RW_AUTHORITY_SCALE))
    for name, number_rw, authority_scale in configurations:
        plot_separate(name, number_rw, authority_scale)
        plot_combined(name, number_rw, B_NOW, "now", "now", authority_scale)
        plot_combined(name, number_rw, B_NEXT, "next", "next", authority_scale)
