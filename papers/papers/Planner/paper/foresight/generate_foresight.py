...
Output: outputs/foresight.png/.pdf
"""
import os, sys
import numpy as np
...
def main():
    ...
    x0 = ADCS.State(w=np.zeros(3), q=normalize(q0), h=np.array([0.0]))
    os0 = P.default_os0()

    res = ADCS.simulate(x=x0, satellite=sat, controller=ctrl, goal=gl,
                        os0=os0, dt=DT, tf=TF)
    run = res.runs[0] if hasattr(res, "runs") else res
    st = ADCS.State.stack(run.state_hist); t = np.asarray(run.time_s, float)
    ...
    fig.tight_layout(); fig.savefig(os.path.join(OUT, "foresight.png"), dpi=150)
    fig.savefig(os.path.join(OUT, "foresight.pdf")); plt.close(fig)

    pre = align_coast_end > align_A + 0.15 or h_at_slew > 3e-4
    print("saved foresight.png/.pdf  | pre-positioning:", pre)
