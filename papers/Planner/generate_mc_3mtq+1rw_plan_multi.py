...
if '--save-plots' in sys.argv:
    os.environ.setdefault('MPLBACKEND', 'Agg')  # Non-interactive for saving
else:
    os.environ.setdefault('DISPLAY', ':0')  # WSLg display
    os.environ.setdefault('MPLBACKEND', 'TkAgg')  # Interactive
import numpy as np
...
if __name__ == "__main__":
    ...
    if TEST_MODE:
        ...
        if args.plot:
            plot_mc_summary(valid, body_boresight=BODY_BORESIGHT, title_prefix="3MTQ+1RW Planner Multi")
            create_close_all_button_window()
            import matplotlib.pyplot as plt
            plt.show()
    else:
