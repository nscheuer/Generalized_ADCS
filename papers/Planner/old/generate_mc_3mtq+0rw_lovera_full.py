...
def run_single_sim(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for i in range(steps):
            ...
            state_hist[ind, :] = x
            u_hist[ind, :] = u
            q0_goal, _ = goal_to_ref(os0=os_state)
            q0_goal_hist[ind, :] = q0_goal
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data("3MTQ+0RW_Lovera_mc_100_1000s_full", full_results, out_dir=OUTPUT_DIR)

        plot_target_tracking_mc(full_results=full_results, body_boresight=np.array([0, 1, 0]), title="3 MTQ + 0 RW Lovera MC:100")
        plot_convergence_histogram_mc(full_results=full_results, body_boresight=np.array([0, 1, 0]), title="3 MTQ + 0 RW Lovera")

        create_close_all_button_window()
    else:
        results = load_data("papers/Planner/output_data/3MTQ+0RW_Lovera_mc_100_20260118_013735")
