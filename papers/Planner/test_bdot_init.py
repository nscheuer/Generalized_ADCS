#!/usr/bin/env python3
"""Test bdot initialization directly."""
...
orb = create_random_circular_orbit(radius_km=7000.0, dt=1, tf=150, use_J2=False, fast=True)
...
print("=== Testing prepareForAlilqr initial trajectory directly ===")
print(f"180° slew: q0={q0}, q_goal={q_goal}\n")
...
for bdot_mode in [0, 1, 2, 3]:
    ...
    settings.dt_tp = 10
    settings.verbosity = True  # Enable verbose to see fallback message
    ...
    print(f"  |U|={ctrl_norm:.3e}, max={ctrl_max:.3e}")
    print(f"  U[:,0:3] =\n{Uset[:3,:3]}")
    print()
