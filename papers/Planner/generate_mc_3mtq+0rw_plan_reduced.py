"""
Monte Carlo for 3MTQ + 0RW with Trajectory Planner - Reduced Attitude 180° Slew.
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        runner = MonteCarloRunner(
            ...
            num_runs=NUM_RUNS,
            max_workers=4,  # Fewer workers for planner (memory intensive)
        )
        ...
        save_data(f"3MTQ+0RW_PLAN_reduced_180slew_mc_{NUM_RUNS}", full_results, out_dir=OUTPUT_DIR)
        
        plot_target_tracking_mc(full_results=valid_results, title=f"3MTQ+0RW Planner Reduced 180° Slew N={len(valid_results)}")
        plot_convergence_histogram_mc(full_results=valid_results, title=f"3MTQ+0RW Planner Reduced 180° Slew N={len(valid_results)}")
        create_close_all_button_window()
    else:
        results = load_data(f"{OUTPUT_DIR}/3MTQ+0RW_PLAN_reduced_180slew_mc_{NUM_RUNS}")
        full_results = results[0] if isinstance(results, tuple) else results
        valid_results = [r for r in full_results if r and r.get("traj_valid", False)]
        plot_target_tracking_mc(full_results=valid_results, title=f"3MTQ+0RW Planner Reduced 180° Slew")
        plot_convergence_histogram_mc(full_results=valid_results, title=f"3MTQ+0RW Planner Reduced 180° Slew")
        create_close_all_button_window()
