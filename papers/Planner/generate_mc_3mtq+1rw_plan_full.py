"""
Monte Carlo for 3MTQ + 1RW with Trajectory Planner - Full Attitude 180° Slew.

This script generates Monte Carlo simulations for:
- 3+1 configuration (3 MTQs + 1 reaction wheel)
- ALTRO trajectory planner with TVLQR tracking
- Full attitude goal (quaternion pointing)
- 180° slew from initial attitude
...
def generate_180_rotation_quat(axis: np.ndarray) -> np.ndarray:
    """Generate quaternion for 180° rotation about given axis."""
    axis = normalize(axis)
...
def run_single_sim(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Hardware Setup - 3 MTQ + 1 RW
        mtq_max = 0.4
        rw_max = 7e-3
        rw_hmax = 16.2e-3
        
        acts = [MTQ(axis=j, max_torque=mtq_max) for j in MathConstants.unitvecs]
        rws = [RW(axis=MathConstants.unitvecs[2], max_torque=rw_max, J=1e-3, h=0, h_max=rw_hmax)]
        acts.extend(rws)
        ...
        # Full attitude goal: 180° rotation from initial quaternion
        q_180 = generate_180_rotation_quat(config["rotation_axis"])
        q_goal = quat_mult(q_180, config["q0"])
        ...
        for i in range(N):
            ...
            state_hist[i, :] = x
            u_hist[i, :] = u
            q_goal_ref, _ = goal_to_ref(t=J2000, os0=os_state)
            boresight_hist[i, :] = q_goal_ref[1:4]
...
def generate_mc_config(run_id: int) -> Dict[str, Any]:
    ...
    rotation_axis = normalize(rng.standard_normal(3))
    ...
    return {
        ...
        "q0": q0,
        "h0": rng.uniform(-0.001, 0.001, size=1),
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data(f"3MTQ+1RW_PLAN_full_180slew_mc_{NUM_RUNS}", full_results, out_dir=OUTPUT_DIR)
        
        plot_target_tracking_mc(full_results=valid_results, title=f"3MTQ+1RW Planner Full 180° Slew N={len(valid_results)}")
        plot_convergence_histogram_mc(full_results=valid_results, title=f"3MTQ+1RW Planner Full 180° Slew N={len(valid_results)}")
        create_close_all_button_window()
    else:
        results = load_data(f"{OUTPUT_DIR}/3MTQ+1RW_PLAN_full_180slew_mc_{NUM_RUNS}")
        full_results = results[0] if isinstance(results, tuple) else results
        valid_results = [r for r in full_results if r and r.get("traj_valid", False)]
        plot_target_tracking_mc(full_results=valid_results, title=f"3MTQ+1RW Planner Full 180° Slew")
        plot_convergence_histogram_mc(full_results=valid_results, title=f"3MTQ+1RW Planner Full 180° Slew")
        create_close_all_button_window()
