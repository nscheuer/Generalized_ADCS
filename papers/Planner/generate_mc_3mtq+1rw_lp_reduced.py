"""
Monte Carlo for 3MTQ + 1RW with LP (PD) Controller - Reduced Attitude 180° Slew.
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data(f"3MTQ+1RW_LP_reduced_180slew_mc_{NUM_RUNS}", full_results, out_dir=OUTPUT_DIR)
        
        plot_target_tracking_mc(full_results=full_results, title=f"3MTQ+1RW LP Reduced 180° Slew N={NUM_RUNS}")
        plot_convergence_histogram_mc(full_results=full_results, title=f"3MTQ+1RW LP Reduced 180° Slew N={NUM_RUNS}")
        create_close_all_button_window()
    else:
        results = load_data(f"{OUTPUT_DIR}/3MTQ+1RW_LP_reduced_180slew_mc_{NUM_RUNS}")
        full_results = results[0] if isinstance(results, tuple) else results
        plot_target_tracking_mc(full_results=full_results, title=f"3MTQ+1RW LP Reduced 180° Slew")
        plot_convergence_histogram_mc(full_results=full_results, title=f"3MTQ+1RW LP Reduced 180° Slew")
        create_close_all_button_window()
