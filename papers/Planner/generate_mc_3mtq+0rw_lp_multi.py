"""
Monte Carlo for 3MTQ + 0RW with LP (PD) Controller - Multi-Goal Scenario.
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data(f"3MTQ+0RW_LP_multigoal_mc_{NUM_RUNS}", full_results, out_dir=OUTPUT_DIR)
        
        plot_target_tracking_mc(full_results=full_results, title=f"3MTQ+0RW LP Multi-Goal N={NUM_RUNS}")
        plot_convergence_histogram_mc(full_results=full_results, title=f"3MTQ+0RW LP Multi-Goal N={NUM_RUNS}")
        create_close_all_button_window()
    else:
        results = load_data(f"{OUTPUT_DIR}/3MTQ+0RW_LP_multigoal_mc_{NUM_RUNS}")
        full_results = results[0] if isinstance(results, tuple) else results
        plot_target_tracking_mc(full_results=full_results, title=f"3MTQ+0RW LP Multi-Goal")
        plot_convergence_histogram_mc(full_results=full_results, title=f"3MTQ+0RW LP Multi-Goal")
        create_close_all_button_window()
