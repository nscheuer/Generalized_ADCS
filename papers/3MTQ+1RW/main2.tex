...
%   |--------------------------------|--------------------------------|-------------|
%   | fig_3p1_cdf.pdf                | 3+1 pointing error CDF         | Results     |
%   | fig_3p1_success_rates.pdf      | Success rate comparison        | Results     |
%   | fig_3p1_error_trajectories.pdf | Error time series              | Results     |
%   | 1W_montecarlo.pdf              | Single-wheel MC results        | Comparison  |
%   | 1W_montecarlo_traj.pdf         | Single-wheel trajectory        | Comparison  |
%   | mtq_montecarlo.pdf             | MTQ-only MC results            | Comparison  |
%   | mtq_montecarlo_traj.pdf        | MTQ-only trajectory            | Comparison  |
%   | angular_error_lovera.pdf       | Lovera controller error        | Baselines   |
%   | angular_error_wisniewski.pdf   | Wisniewski controller error    | Baselines   |
%   | ctrl_lovera.pdf                | Lovera control commands        | Baselines   |
%   | ctrl_wisniewski.pdf            | Wisniewski control commands    | Baselines   |
%
% DATA FROM ALTRO TUNING (research/ALTRO_TUNING_NOTES.md):
%   - Best 500s config: solve time 51.5s, final error 0.08°
%   - Optimal parameters: dt_tp=100, angle=100, angle_N=50000
%   - Bug fixes documented (B-field units, quaternion order)
%
% BC2 PARAMETERS (from ADCS/satellite_factory/satellites/create_cubesats.py):
%   - Mass: 4 kg
%   - Inertia: J = [[0.0314, 0.0001, -0.0067],
%                   [0.0001, 0.0341, -0.0001],
%                   [-0.0067, -0.0001, 0.0100]] kg·m²
%   - Actuators: ISIS MTQ board + CubeWheel SmallPlus (z-axis)
...
%
% ============================================================
% CRITICAL EXPERIMENTS TO RESOLVE DATA DISCREPANCY (RUN FIRST!)
% ============================================================
%
% ISSUE: Abstract claims 2.3° mean (PD) and 0.05° (planner), but current data shows:
%   - experiment_outputs/3p1_paper/ (200s, PD): 63° mean for 3+1
%   - papers/3MTQ+1RW/output_data/ (500s, ALTRO): 30° mean for 3+1
%   - Thesis claims: 96% within 1°, mean 0.45°
%
% HYPOTHESIS: The difference is GOAL TYPE (reduced vs full attitude)
%
% ============================================================
% EXPERIMENT 0: DIAGNOSE THE DISCREPANCY (RUN IMMEDIATELY)
% ============================================================
%
% EXP-0A: REDUCED-ATTITUDE Goal (thesis configuration)
%   Purpose: Verify thesis results are reproducible
%   Spacecraft: BC2 (create_beavercube2_cubesat())
%   Orbit: 400km circular, 51.6° inclination
%   Duration: 1000s
%   Trials: 10 (quick validation), then 100 (full MC)
%   Goal type: REDUCED-ATTITUDE (align body-y with target direction)
%              goal = ReducedAttitudeGoal(body_axis=[0,1,0], target_dir=random_unit_vector)
%   Controller: ALTRO + TVLQR
%   ALTRO params (from ALTRO_TUNING_NOTES.md Session 5):
%     ps.cost_main.ang_cost_func_type = 0  # LINEAR - critical for early convergence!
...
%   TVLQR gains (critical - see ALTRO_TUNING_NOTES.md):
%     cost_tvlqr.angle = 1e4
%     cost_tvlqr.angle_N = 1e5
%     cost_tvlqr.ang_vel = 1e4
%     cost_tvlqr.ang_vel_N = 1e5
%     cost_tvlqr.control_mult = 1e4  # CRITICAL: prevents oscillations
%   Output data:
%     - quaternion(t), angular_velocity(t), pointing_error(t)
%     - final_error_deg, settle_time_to_5deg, settle_time_to_1deg
%     - altro_solve_time_s, altro_iterations
%   Expected result: ~1° mean, 90%+ within 5° (matching thesis)
%
% EXP-0B: FULL-ATTITUDE Goal (current configuration)
%   Purpose: Understand why current data shows 30° error
%   Same as EXP-0A but:
%   Goal type: FULL-ATTITUDE (exact quaternion)
%              goal = QuaternionGoal(q_target=random_quaternion)
%   Expected result: ~30° mean (matching current data)
%
%   IF EXP-0A matches thesis AND EXP-0B matches current data:
%     CONCLUSION: Goal type explains discrepancy
%     ACTION: Clearly document in paper that thesis results use reduced-attitude
%
%   IF BOTH fail to match:
%     CONCLUSION: Something else changed (bug, parameters, orbit)
%     ACTION: Debug systematically
%
% ============================================================
% EXPERIMENT 0C: PD-ONLY BASELINE (no planner)
% ============================================================
%   Purpose: Generate correct numbers for Table 1 (PD results)
%   Spacecraft: BC2
%   Orbit: 400km circular, 51.6° inclination
%   Duration: 1000s (NOT 200s like current data!)
%   Trials: 100
%   Configs: 3+0, 3+1, 3+3
%   Goal type: REDUCED-ATTITUDE (body-y aligned with target)
%   Controller: Quaternion PD (no planner)
%   PD gains (need tuning recommendation):
%     kp = [TBD based on config - see thesis values]
%     kd = [TBD based on config]
%     For 3+0: kp=0.0001, kd=0.001 (weak due to MTQ limits)
%     For 3+1: kp=0.001, kd=0.01 (moderate)
%     For 3+3: kp=0.01, kd=0.1 (aggressive)
%   Allocation: LP (direction-preserving)
%   Output data:
%     - error_trajectory_deg[t] for each trial
%     - histogram of final_error_deg
%     - CDF of pointing_error
%   Output figures:
%     - fig_pd_cdf.pdf: CDF comparing 3+0, 3+1, 3+3
%     - fig_pd_histogram.pdf: Error distribution
%     - fig_pd_trajectories.pdf: Representative error vs time
%
% ============================================================
% FIGURE WEAKNESSES IN CURRENT DATA
% ============================================================
%
% WEAKNESS 1: thesis_figures/fig_3p1_cdf.pdf
%   Issue: May be from different parameters than current code
%   Action: Regenerate with EXP-0C to ensure consistency
%
% WEAKNESS 2: thesis_figures/fig_3p1_error_trajectories.pdf
%   Issue: Only shows select "good" trajectories, not representative
%   Action: Regenerate showing 10 random trajectories overlaid
%
% WEAKNESS 3: No planner vs PD comparison figure
%   Issue: Key claim is planner improves 3+1, but no direct comparison
%   Action: Generate figure with:
%     - 3+1 PD error trajectory
%     - 3+1 ALTRO error trajectory
%     - Same initial conditions, same goal
%
% WEAKNESS 4: No figure showing "emergent spinning behavior"
%   Issue: Abstract mentions spin for disturbance rejection but no figure
%   Action: Generate figure showing:
%     - Angular velocity magnitude vs time
%     - Pointing error vs time (showing spin maintains pointing)
%     - Control commands showing MTQ+RW coordination
%
% ============================================================
...
% ============================================================
% FIGURE: Actuator Configuration Comparison
% STATUS: NEEDS GENERATION - Create diagram showing 3+0, 3+1, 3+3 configs
% Suggested content: 3D spacecraft view with actuator placement marked
% Could adapt from existing CAD or create schematic
% ============================================================
\begin{figure}[h]
    \centering
    \fbox{\parbox{0.9\columnwidth}{\centering\vspace{1.5cm}
    \textbf{FIGURE TO GENERATE}\\
    Content: Actuator configuration comparison (3+0, 3+1, 3+3)\\
    Show: MTQ placement (3-axis), RW axis orientation\\
    Note: No existing figure - needs creation
    \vspace{1.5cm}}}
    \caption{Actuator configurations evaluated: (a) 3+0 with three orthogonal magnetorquers, (b) 3+1 adding a single z-axis reaction wheel, (c) 3+3 with three orthogonal reaction wheels plus magnetorquers for desaturation.}
    \label{fig:actuator_configs}
\end{figure}
...
% The numbers in Tables 2-4 are from THESIS RESULTS (planning.tex).
% Current codebase produces DIFFERENT results:
%   - experiment_outputs/3p1_paper/: 200s tests, PD only → 63° for 3+1
%   - papers/3MTQ+1RW/output_data/: 500s ALTRO+TVLQR → 30° mean
%   - Thesis claims: 96% within 1°, 0.45° mean
%
% BEFORE SUBMISSION: Re-run experiments with:
%   - 1000s duration, 100 trials
%   - REDUCED-ATTITUDE goals (thesis used this!)
%   - Verify tables match regenerated data
% ============================================================
...
% ============================================================
% FIGURE: Error CDF Comparison
% STATUS: AVAILABLE in experiment_outputs/thesis_figures/fig_3p1_cdf.pdf
% NOTE: This figure is from thesis_figures, verify it matches claimed numbers
% ============================================================
\begin{figure}[h]
    \centering
    % \includegraphics[width=\columnwidth]{../experiment_outputs/thesis_figures/fig_3p1_cdf.pdf}
    % TODO: Uncomment above line and verify path when compiling
    \fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}
    \textbf{FIGURE PLACEHOLDER}\\
    File: experiment\_outputs/thesis\_figures/fig\_3p1\_cdf.pdf\\
    Content: CDF of pointing errors for 3+0, 3+1, 3+3\\
    Status: AVAILABLE - verify numbers match Table~\ref{tab:pd_results}
    \vspace{2cm}}}
    \caption{Cumulative distribution of pointing errors across 100 Monte Carlo trials. The 3+1 configuration (blue) achieves sub-degree pointing in 73\% of trials, compared to 15\% for 3+0 and 100\% for 3+3.}
    \label{fig:pd_cdf}
\end{figure}

% ============================================================
% FIGURE: Error Time Series
% STATUS: AVAILABLE in experiment_outputs/thesis_figures/fig_3p1_error_trajectories.pdf
% ============================================================
\begin{figure}[h]
    \centering
    % \includegraphics[width=\columnwidth]{../experiment_outputs/thesis_figures/fig_3p1_error_trajectories.pdf}
    \fbox{\parbox{0.9\columnwidth}{\centering\vspace{2cm}
    \textbf{FIGURE PLACEHOLDER}\\
    File: experiment\_outputs/thesis\_figures/fig\_3p1\_error\_trajectories.pdf\\
    Content: Pointing error vs time for representative trials\\
    Status: AVAILABLE
    \vspace{2cm}}}
    \caption{Representative pointing error trajectories for each configuration. The 3+1 system converges within 200 seconds for most initial conditions, while 3+0 often fails to converge within the 1000-second window.}
