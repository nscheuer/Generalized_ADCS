...
def run_single_sim(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 6. Controller
        GAMMA = 100
        controller = MTQ_w_RW_QPG(est_sat=real_sat, p_gain=0.00005, d_gain=0.001, gamma=GAMMA, c_gain=0.001, h_target=np.array([0.004, 0.0, 0.0]))
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data("3MTQ+1RW_QPG10_gamma32_mc_100", full_results, out_dir=OUTPUT_DIR)
        
        plot_target_tracking_mc(full_results=full_results, title="3 MTQ + 1 RW QPG $\\gamma = 10$ MC:32")
        plot_convergence_histogram_mc(full_results=full_results, title="3 MTQ + 1 RW QPG $\\gamma = 10$ MC:32")
