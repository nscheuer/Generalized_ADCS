...
"""
Comparison between 3 MTQ + 3 RW MTQ_w_RW control
...
def run_single_sim(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 5. Controller
        controller = MTQ_w_RW_LP(est_sat=real_sat, p_gain=0.0002, d_gain=0.004, c_gain=0.002, h_target=np.array([0.0, 0.0, 0.0]))
...
def generate_mc_config(run_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "h0": rng.uniform(-0.0001, 0.0001, size=3),
...
if __name__ == "__main__":
    ...
    if RUN_MC:
        ...
        save_data("3MTQ+3RW_MTQ_w_RW_LP_mc_100_1000s", full_results, out_dir=OUTPUT_DIR)

        plot_target_tracking_mc(full_results=full_results, body_boresight=np.array([0, 1, 0]), title="3 MTQ + 3 RW MTQ_w_RW_LP MC:100")
        plot_convergence_histogram_mc(full_results=full_results, body_boresight=np.array([0, 1, 0]), title="3 MTQ + 3 RW MTQ_w_RW_LP")
