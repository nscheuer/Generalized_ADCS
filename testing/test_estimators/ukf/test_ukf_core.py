...
def test_uakf_constructor_validates_reduced_covariance_shape():
    ...
    with pytest.raises(ValueError):
        make_ukf(est_sat, x_hat=x_hat, P_hat=np.eye(x_hat.augmented_size), Q_hat=np.eye(x_hat.augmented_size), quat_as_vec=False)
...
def test_uakf_constructor_validates_full_quaternion_covariance_shape():
    ...
    with pytest.raises(ValueError):
        make_ukf(est_sat, x_hat=x_hat, P_hat=np.eye(x_hat.augmented_size - 1), Q_hat=np.eye(x_hat.augmented_size - 1), quat_as_vec=True)
