...
def test_weight_formulas_change_with_scaling_parameters():
    ...
    _, _, wts_before, _, _ = ukf.make_pts_and_wts(ukf.x_hat.as_estimator_array(), [True] * len(est_sat.attitude_sensors))
    ukf.al = 0.2
    ukf.bet = 1.5
    ukf.kap = 2.0
    _, _, wts_after, _, _ = ukf.make_pts_and_wts(ukf.x_hat.as_estimator_array(), [True] * len(est_sat.attitude_sensors))
...
def test_reset_updates_dt_and_process_covariance():
    ...
    assert np.allclose(ukf.x_hat.as_estimator_array(), x_hat.as_estimator_array())
