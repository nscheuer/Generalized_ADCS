...
def test_make_pts_and_wts_returns_expected_counts_and_weight_sums():
    ...
    L, pts, wts_m, wts_c, sig0 = ukf.make_pts_and_wts(ukf.x_hat.val.copy(), [True] * len(est_sat.attitude_sensors))

    assert len(pts) == 2 * L + 1
    assert wts_m.shape == (2 * L + 1,)
    assert wts_c.shape == (2 * L + 1,)
    assert sig0.shape == (2 * L + 1, ukf.x_hat.val.size)
...
def test_make_pts_and_wts_augmented_dimension_includes_control_noise():
    ...
    L, _, _, _, _ = ukf.make_pts_and_wts(ukf.x_hat.val.copy(), [True] * len(est_sat.attitude_sensors))
...
def test_make_pts_and_wts_center_weight_matches_formula():
    ...
    L, _, wts_m, wts_c, _ = ukf.make_pts_and_wts(ukf.x_hat.val.copy(), [True] * len(est_sat.attitude_sensors))
...
def test_make_pts_and_wts_returns_control_only_offsets_for_non_state_block():
    ...
    L, pts, _, _, _ = ukf.make_pts_and_wts(ukf.x_hat.val.copy(), [True] * len(est_sat.attitude_sensors))
    state_sigma_count = 2 * ukf.x_hat.cov.shape[0]
    first_control_point = pts[1 + state_sigma_count]

    assert np.allclose(first_control_point[0], ukf.x_hat.val)
...
def test_make_pts_and_wts_keeps_quaternions_normalized_in_error_state_mode():
    ...
    _, pts, _, _, sig0 = ukf.make_pts_and_wts(ukf.x_hat.val.copy(), [True] * len(est_sat.attitude_sensors))
...
def test_make_pts_and_wts_zero_covariance_raises():
    ...
    with pytest.raises(np.linalg.LinAlgError):
        ukf.make_pts_and_wts(ukf.x_hat.val.copy(), [True] * len(est_sat.attitude_sensors))
