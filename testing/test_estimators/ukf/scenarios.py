...
def bias_scenario() -> SimulationResult:
    ...
    ukf = make_ukf(est_sat, x_hat=make_estimate_guess(est_sat), dt=5.0, cross_term=False)
...
def startracker_dropout_scenario() -> SimulationResult:
    ...
    real_sensors = [
        *[MTM(axis=axis, noise=Noise(noise=0.0, std_noise=1.0e-8)) for axis in MathConstants.unitvecs],
        *[Gyro(axis=axis, noise=Noise(noise=0.0, std_noise=1.0e-5)) for axis in MathConstants.unitvecs],
        StarTracker(
    ...
    real_sat, est_sat = make_satellites(sensors=real_sensors, estimated_sensors=est_sensors, actuators=make_mtqs(), estimated_actuators=make_mtqs())
    ukf = make_ukf(est_sat, dt=5.0, cross_term=False)
