...
def test_update_with_finite_measurements_keeps_state_covariance_and_square_root_finite():
    real_sat, est_sat = make_satellites(sensors=make_baseline_sensors(), estimated_sensors=make_baseline_sensors())
    srukf = make_srukf(est_sat)
...
def test_update_raises_linalgerror_when_triangular_solver_fails(monkeypatch):
    ...
    srukf = make_srukf(est_sat)
    sensors = real_sat.noiseless_sensor_readings(make_state(), make_orbital_state())
    monkeypatch.setattr(srukf_module.scipy.linalg, "solve_triangular", lambda *args, **kwargs: (_ for _ in ()).throw(ValueError("boom")))
