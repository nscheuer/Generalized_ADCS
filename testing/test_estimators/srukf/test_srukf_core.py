...
def test_srukf_constructor_validates_reduced_covariance_shape():
    ...
    with pytest.raises(ValueError):
        make_srukf(est_sat, x_hat=x_hat, P_hat=np.eye(x_hat.augmented_size), Q_hat=np.eye(x_hat.augmented_size), quat_as_vec=False)
...
def test_srukf_constructor_validates_full_quaternion_covariance_shape():
    ...
    x_hat = make_estimate_guess(est_sat)
    with pytest.raises(ValueError):
        make_srukf(est_sat, x_hat=x_hat, P_hat=np.eye(x_hat.augmented_size - 1), Q_hat=np.eye(x_hat.augmented_size - 1), quat_as_vec=True)
...
def test_srukf_sat_match_updates_bias_state():
    ...
    srukf = make_srukf(est_sat)
    state = srukf.x_hat.copy()
    state.sens_bias[:3] = np.array([1.0e-3, -2.0e-3, 3.0e-3])
...
def test_srukf_update_returns_normalized_quaternion_symmetric_covariance_and_square_root():
    ...
    assert np.isclose(np.linalg.norm(out.q), 1.0)
