...
@pytest.mark.slow
def test_bdot(verbose: bool = False, tf: float = 1000, dt: float = 10, real_orbit: bool = False) -> Union[np.ndarray, np.ndarray, List[Orbital_State], np.ndarray, np.ndarray]:
    np.random.seed(1)
    t0 = 0
    N = int((tf-t0)/dt)

    max_torque = 0.1
    acts = [MTQ(axis=j, max_torque=max_torque) for j in MathConstants.unitvecs]
    ...
    ind = 0
    steps = int((tf - t0)/dt)
    for step in tqdm(range(steps), desc="Simulating BDot"):
...
def test_bdot_w_rw(verbose: bool = False, tf: float = 1000, dt: float = 10, real_orbit: bool = False) -> Union[np.ndarray, np.ndarray, List[Orbital_State], np.ndarray, np.ndarray]:
    np.random.seed(1)
    t0 = 0
    N = int((tf-t0)/dt)
...
def plot_bdot(verbose: bool = False, tf: float = 1000, dt: float = 10, real_orbit: bool = False) -> None:
    (time_hist, state_hist, os_hist, sensor_hist, u_hist) = test_bdot(verbose=verbose, tf=tf, dt=dt, real_orbit=real_orbit)
...
def plot_bdot_w_rw(verbose: bool = False, tf: float = 1000, dt: float = 10, real_orbit: bool = False) -> None:
    (time_hist, state_hist, os_hist, sensor_hist, u_hist) = test_bdot_w_rw(verbose=verbose, tf=tf, dt=dt, real_orbit=real_orbit)
...
if __name__ == "__main__":
    plot_bdot_w_rw(verbose=False, tf = 100, dt = 1, real_orbit= False)