"""Fast dynamics performance checks.
...
normalizing by a small CPU/Numpy calibration workload:
    ...
    python testing/benchmarks/benchmark_dynamics.py --compare

To refresh the baseline after an intentional performance change:

    python testing/benchmarks/benchmark_dynamics.py --update-baseline
"""
...
RESULT_PATH = REPO_ROOT / "testing" / "artifacts" / "benchmarks" / "dynamics_current.json"
...
def _make_reference_case() -> dict[str, object]:
    os0 = _reference_os()
    os1 = os0.propagate_orbit_rk4(dt=1.0, zonal_J=2, fast=True)

    sat_plain = Satellite()
    sat_mtq = Satellite(actuators=_make_mtqs())
    ...
    u_full = np.concatenate([u_mtq, u_rw])
...
def _make_benchmarks() -> list[Benchmark]:
    return [
        Benchmark(
            name="dynamics_core_plain",
            func=lambda: _REFERENCE["sat_plain"].dynamics_core(
                x=_REFERENCE["x_plain"],
                u=np.array([], dtype=float),
                orbital_state=_REFERENCE["os0"],
            ),
            max_regression=1.50,
        ),
        Benchmark(
            name="dynamics_core_mtq",
            func=lambda: _REFERENCE["sat_mtq"].dynamics_core(
                x=_REFERENCE["x_plain"],
                u=_REFERENCE["u_mtq"],
                orbital_state=_REFERENCE["os0"],
            ),
        ...
        Benchmark(
            name="dynamics_core_reaction_wheels",
            func=lambda: _REFERENCE["sat_rw"].dynamics_core(
                x=_REFERENCE["x_rw"],
                u=_REFERENCE["u_rw"],
                orbital_state=_REFERENCE["os0"],
            ),
        ...
        Benchmark(
            name="noiseless_rk4_full_stack",
            func=lambda: _REFERENCE["sat_full"].noiseless_rk4(
                _REFERENCE["x_rw"],
                _REFERENCE["u_full"],
                1.0,
                _REFERENCE["os0"],
...
def run_benchmarks(samples: int) -> dict:
    ...
    results = {
        "schema_version": 1,
        "description": "Dynamics fast benchmark results normalized by calibration_seconds.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark ADCS satellite dynamics paths.")
