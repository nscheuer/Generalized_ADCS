...
## Session 7: Comparison with NSSR_3+1 Branch (2026-01-21)

### Branch Comparison

Compared our PKMN_3+1 branch with NSSR_3+1 branch (coworker's).

#### TVLQR Fix Status
Both branches now have the same quaternion error fix:
```python
# Both branches now use (correct):
q_err = quat_diff(x_ref[3:7], x_curr[3:7])
dx[3:6] = quat_to_vec3(q_err)
```
The fix was independently discovered/applied.

#### Default Settings Differences

| Setting | PKMN_3+1 (ours) | NSSR_3+1 |
|---------|-----------------|----------|
| `use_dynamics_hess` | 1 | 0 |
| `grad_tol` | 1e-4 | 1e-3 |
| `ilqr_cost_tol` | 1e-2 | 1e-1 |
| `c_max` | 0.0002 | 0.002 |
| `use_full_cost_hessian` | True | False |

Our branch uses tighter tolerances and includes Hessians by default.

#### NSSR's debug_altro_bc2_3+1.py Settings

Their test script (`debug/debug_controllers/debug_altro/debug_altro_bc2_3+1.py`):
- `bdot_on=0`, `dt_tp=100`, `dt_tvlqr=1`
- `cost_main`: angle=1e3, angle_N=1e6, ang_vel=1e3, ang_vel_N=1e5, control_mult=1.0, ang_cost_func_type=2
- `cost_tvlqr`: angle=1e2, angle_N=1e3, **ang_vel=1e6, ang_vel_N=1e8**, control_mult=1.0
- **body_boresight = [0, 1, 0]** (Y-axis, not Z-axis!)
- Initial conditions: w0=[0,0,0], q0=[1,0,0,0] (identity)

#### Known Issues in NSSR_3+1

Running their `debug_altro_bc2_3+1.py` shows planner instability:
```
WARNING: Extreme constraint violation: inf
WARNING: Qkuu has nan or inf
```

This is likely due to:
1. `bdot_on=0` with trivial initial conditions (identity quaternion) leading to poor initial trajectory guess
2. Low `control_mult=1.0` in cost settings
3. Geodesic cost (type=2) with large dt_tp=100

#### Recommendations

For stable ALTRO planning:
1. Use `bdot_on=2` (smart bdot) for initial trajectory guess
2. Use `dt_tp=50` instead of `dt_tp=100` for better resolution
3. Use `control_mult > 1` to regularize the control Hessian
4. Consider linear cost (type=0) for monotonic convergence

---

## Session 8: Unit Fix and NSSR Comparison (2026-01-21)

### Unit Convention Fixed

**Problem:** The codebase had inconsistent unit handling:
- Our `/1000` fix in `get_b_eci()` assumed R in meters
- But gravity gradient, Skyfield, and C++ planner all expect R in km

**Solution:** Reverted the `/1000` fix and standardized on km:
- `R` should be in **kilometers** (e.g., `R = 7000 * np.array([...])`)  
- `V` should be in **km/s** (e.g., `V = np.array([8, 0, 0])`)
- This matches NSSR and the rest of the codebase

### Files Changed
- `ADCS/orbits/orbital_state.py`: Removed `/1000` in `get_b_eci()`
- `ADCS/orbits/orbit.py`: Removed `/1000` in `get_b_eci_orbit()`
- All debug scripts updated to use km units

### NSSR Comparison Findings

**Why NSSR's code hangs (same as ours):**

The planner hangs when:
1. `bdot_on=0` (no initial trajectory guess via bdot)
2. `omega = [0, 0, 0]` (exactly zero angular velocity)

This is a numerical issue in the C++ trajOpt - it can't generate an initial trajectory when there's no angular motion and no bdot guess.

**NSSR's code has NOT been successfully tested** with `real_orbit=True` and `tf=500` - it hangs just like ours did.

### Solutions

1. **Use `bdot_on=2`** (smart mode) - generates initial trajectory guess
2. **Use small non-zero omega** (e.g., `[0.01, 0.01, 0.01]` rad/s) with `bdot_on=0`
...
```python
# Initial conditions - use small non-zero omega
w0 = np.array([0.01, 0.01, 0.01])  # ~1 deg/s total
...
# Planner settings
ps = PlannerSettings(est_sat=sat, bdot_on=2, dt_tp=30, dt_tvlqr=1)
# OR
ps = PlannerSettings(est_sat=sat, bdot_on=0, dt_tp=100, dt_tvlqr=1)  # with non-zero omega

# B-field should be ~35 uT for LEO
```

### Performance Results (60s trajectory, non-zero omega)

| bdot_on | dt_tp | Solve Time | Converges |
|---------|-------|------------|-----------|
| 0 | 100 | ~3s | ✓ |
| 2 | 30 | ~5s | ✓ |

Both configurations work reliably with proper initial conditions.
