...
def create_covariance_matrices(est_sat: EstimatedSatellite, dt: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    P_est = block_diag(np.eye(3)*(0.01)**2.0, np.eye(3)*3, np.eye(3)*(1e-8)**2.0, np.eye(3)*(0.2*np.pi/180.0)**2.0, np.eye(2)*(0.06)**2.0)
...
def plot_results(time_hist, state_hist, est_state_hist, os_hist, sensor_hist, clean_sensor_hist, u_hist, cov_hist) -> None:
    ...
    plot_bias_comparison(time_hist, state_hist[:,13:15], est_state_hist[:,13:15], 
                        "Real vs Estimated SunPair Bias", "T/s")
...
def main() -> None:
    ...
    x_hat = np.zeros(15) #7 state, 3 MTM, 3 gyro, 2 SunPair
    x_hat[3] = 1
