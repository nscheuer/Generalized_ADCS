...
def create_covariance_matrices(est_sat: EstimatedSatellite, dt: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Q_est = block_diag(Q_dyn_block, Q_mtm)
    P_est = block_diag(np.eye(3)*(0.01)**2.0, np.eye(3)*3, np.eye(3)*(1e-8)**2.0)
...
def main() -> None:
    ...
    x_hat = np.zeros(10) #7 state, 3 MTM
    x_hat[3] = 1
    ...
    for step in tqdm(range(steps), desc="Simulating ukf"):
        ...
        state_hist[ind,:] = np.concatenate([x, real_mtm_biases])
