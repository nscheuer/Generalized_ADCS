...
def create_beavercube1_cubesat(estimated: bool = True):
    ...
    # Actuators
    mtqs: List[MTQ] = create_isis_magnetorquer_board(bias=triple_bias, noise=triple_noise, estimate_bias=False)
    
    # Sensors
    mtms: List[MTM] = create_isis_magnetometer(bias=triple_bias, noise=triple_noise, estimate_bias=False)
    gyros: List[Gyro] = create_ICM20948_IMU(bias=triple_bias, noise=triple_noise, estimate_bias=False)
    solar_panel_1 = create_Clydespace_3U_array(axis=np.array([1, 0, 0]), bias=Bias(), noise=Noise(), estimate_bias=False)
    solar_panel_2 = create_Clydespace_3U_array(axis=np.array([0, 1, 0]), bias=Bias(), noise=Noise(), estimate_bias=False)
