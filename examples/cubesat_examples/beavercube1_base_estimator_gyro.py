...
from ADCS.satellite_hardware.errors import ErrorMode, Noise, Bias
...
def create_covariance_matrices(est_sat: EstimatedSatellite, dt: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Q_est = block_diag(Q_dyn_block, Q_gyro)
    P_est = block_diag(np.eye(3)*(0.01)**2.0, np.eye(3)*3, np.eye(3)*(0.2*np.pi/180.0)**2.0)
...
def main() -> None:
    ...
    x_hat = np.zeros(10) #7 state, 3 gyro
    x_hat[3] = 1
