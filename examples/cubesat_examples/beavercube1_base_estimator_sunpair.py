...
def create_beavercube1_cubesat(estimated: bool = True):
    ...
    # Sensors
    mtms: List[MTM] = create_isis_magnetometer(bias=triple_bias, estimate_bias=False)
    gyros: List[Gyro] = create_ICM20948_IMU(bias=triple_bias, estimate_bias=False)
    solar_panel_1 = create_Clydespace_3U_array(axis=np.array([1, 0, 0]), estimate_bias=True)
    solar_panel_2 = create_Clydespace_3U_array(axis=np.array([0, 1, 0]), estimate_bias=True)
...
def create_covariance_matrices(est_sat: EstimatedSatellite, dt: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Q_sun = np.eye(2)*(sun_bsr)**2.0 * dt

    Q_est = block_diag(Q_dyn_block, Q_sun)
    P_est = block_diag(np.eye(3)*(0.01)**2.0, np.eye(3)*3, np.eye(2)*(0.06)**2.0)
...
def main() -> None:
    ...
    x_hat = np.zeros(9) #7 state, 2 sunpair
    x_hat[3] = 1
    ...
    for step in tqdm(range(steps), desc="Simulating ukf"):
        ...
        state_hist[ind,:] = np.concatenate([x, real_sun_biases])
