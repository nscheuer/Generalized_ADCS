...
def main():
    ...
    covariance = ADCS.Covariance(
        np.diag([0.01**2] * 3 + [0.15**2] * 3 + [0.05**2] * 3 + [0.5**2] * 3),
        form="sqrt", coordinates="state_tangent", psd_policy="allow_indefinite",
    )
    process_noise = ADCS.Covariance(
        np.diag([1.0e-16] * 3 + [1.0e-8] * 3 + [1.0e-12] * 3 + [1.0e-10] * 3),
        form="sqrt", coordinates="state_tangent", psd_policy="allow_indefinite",
    )
    ...
    ADCS.plot(results, ADCS.plots.AttitudePlot(sources=["real", "estimated"]), layout=(1, 1), title="Advanced SRUKF")
    ADCS.plot(
        results,
        ...
        layout=(2, 3),
        title="Advanced SRUKF: Gyro Bias and Lumped Disturbance",
    )
    plt.show()
