...
def main():
    ...
    x_hat = ADCS.EstimatorState(
        ...
        int_cov=np.diag([1.0e-16] * 3 + [0.0, 1.0e-8, 1.0e-8, 1.0e-8]),
    )
    estimator = ADCS.EKF(
        estimated, x_hat, dt=dt,
        unmodeled_dynamics_psd=np.array([1.0e-16 / dt] * 3 + [0.0, 1.0e-8 / dt, 1.0e-8 / dt, 1.0e-8 / dt]),
    )
    ...
    ADCS.plot(results, ADCS.plots.AttitudePlot(sources=["real", "estimated"]), layout=(1, 1), title="EKF")
    ADCS.plot(
        results,
        ...
        ADCS.plots.IlluminationPlot(), layout=(2, 2), title="EKF",
    )
    plt.show()
