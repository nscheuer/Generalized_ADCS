...
def main():
    ...
    x_hat = ADCS.EstimatorState(
        ...
        cov=np.diag([0.01**2] * 3 + [0.15**2] * 4 + [0.05**2] * 3 + [0.5**2] * 3),
        int_cov=np.diag(
            [1.0e-16] * 3
            + [0.0, 1.0e-8, 1.0e-8, 1.0e-8]
            + [1.0e-12] * 3
            + [1.0e-10] * 3
        ),
    )
    ...
    ADCS.plot(results, ADCS.plots.AttitudePlot(sources=["real", "estimated"]), layout=(1, 1), title="Advanced EKF")
    ADCS.plot(
        results,
        ...
        layout=(2, 3),
        title="Advanced EKF: Gyro Bias and Lumped Disturbance",
    )
    plt.show()
