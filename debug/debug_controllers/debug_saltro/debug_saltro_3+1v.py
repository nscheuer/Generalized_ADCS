...
def _run_open_loop_trajopt(
    ...
    # Use C++ PlannerSettings directly to match debug_3_1_slew90_dt10_vector.py exactly
    ...
    # Vector pointing version: 90-degree slew goal with constant direction
    # Initial error: identity quaternion has z-axis along z-direction = 90 degrees from goal
    # Goal: body z-axis must point toward inertial y-direction (negative)
    vector_goal = np.array(
        [
            [np.nan, np.nan],
            [0.0, 0.0],  # x component (constant goal)
            [-1.0, -1.0],  # y component (constant goal, negative)
...
def debug_saltro(
    ...
    for k in tqdm(range(n_out), desc="Simulating SALTRO open-loop (vector)"):
        os_k = orb.get_os(J2000=float(jtime[k]))
        os_hist.append(os_k)
        sensor_hist[k, :] = real_sat.sensor_readings(x=state_hist[k, :], os=os_k)

    if verbose:
        print("SALTRO trajOpt succeeded (open-loop, vector goal)")
        print(f"N out={n_out}")
...
def debug_saltro_closed_loop(
    ...
    for k in tqdm(range(n_out), desc="Simulating SALTRO closed-loop (vector)"):
        os_k = orb.get_os(J2000=float(jtime[k]))
        os_hist.append(os_k)
        sensor_hist[k, :] = real_sat.sensor_readings(x=state_hist[k, :], os=os_k)
    ...
    if verbose:
        print("SALTRO trajOpt succeeded (closed-loop TVLQR, vector goal)")
...
def _plot_debug_run(
    ...
    # Compute vector error: angle between boresight (in inertial) and goal vector
    ...
    ax.set_title("Quaternion (Vector Goal)")
    ...
    fig.suptitle(f"SALTRO {tag} (Vector Goal) Summary", fontsize=14, fontweight="bold")
    plt.show()
    print(f"Rendered {tag} summary figure (vector goal)")
