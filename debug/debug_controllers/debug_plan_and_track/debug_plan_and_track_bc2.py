...
def test_plan_and_track_lqr(
    ...
    # ============ OPTIMIZED HYBRID CONFIG ============
    # Combines NSSR's fast convergence settings with linear cost for best accuracy.
    # 
    # Performance comparison (500s trajectory):
    #   - Original PKMN config: 318s wall time, 71.53° final error
    #   - NSSR config:          98s wall time,  4.64° final error  
    #   - This hybrid config:  109s wall time,  0.20° final error  <-- BEST
    #
    # Key insights:
    #   1. dt_tp can be large for speed, but sanity check in plan_and_track_base.py
    #      ensures it's not too large (N >= 4 per 60s segment, so dt_tp <= 20s)
    #   2. bdot_on=0 (skip) is faster than bdot_on=3 (smart)
    #   3. Linear cost (type=0) gives best accuracy but needs lower weights
    #   4. Full Hessians (use_full_cost_hessian=True) speed up convergence
    #   5. High running costs (1e6) slow down solver - use 1e3 instead
    #
    print("Setting up trajectory planner...")
    planner_settings = PlannerSettings(
        est_sat=real_sat,
        bdot_on=0,          # Skip bdot for speed (vs bdot_on=3 which is 3x slower)
        dt_tp=10,           # dt_tp=10 with linear cost is fastest AND most accurate
        dt_tvlqr=dt,
    )
    planner_settings.verbosity = verbose

    # Use full Hessians for faster convergence (from NSSR)
    planner_settings.cost_main.use_full_cost_hessian = True
    planner_settings.pass1.regularization.use_dynamics_hess = 1

    # Cost weights: NSSR values with linear cost function for best accuracy
    # NOTE: High running costs (1e6) slow down solver significantly!
    from ADCS.controller.helpers.planner_subsettings import CostWeights
    planner_settings.cost_main = CostWeights(
        angle=1e3,              # Lower than 1e6 for faster solve
        angle_N=1e6,            # High terminal cost for final accuracy
        ang_vel=1e3,
        ang_vel_N=1e5,          # High terminal ang_vel cost (NSSR value)
        ang_vel_mag=0.0,
        ang_vel_mag_N=0.0,
        control_mult=1.0,
        ang_cost_func_type=0,   # Linear cost - best for <0.5° final error
    )
    
    # Iteration limits (from NSSR - slightly lower is fine)
    ...
    # TVLQR uses geodesic cost (type=2) for stable tracking
    planner_settings.cost_tvlqr = CostWeights(
        ...
        ang_vel_mag_N=0.0,
        control_mult=1.0,
        ang_cost_func_type=2,   # Geodesic for TVLQR tracking
    )
...
if __name__ == "__main__":
    plot_plan_and_track_lqr(
        verbose=True,   # Show planner output
        tf=500,         # 500s trajectory (same as comparison test)
        dt=1,
        dt_planning=10, # Not used anymore - dt_tp is hardcoded to 10 in planner settings
        real_orbit=False,  # Use fake orbit for faster testing (True for real J2 orbit)
        seed=42,
    )
