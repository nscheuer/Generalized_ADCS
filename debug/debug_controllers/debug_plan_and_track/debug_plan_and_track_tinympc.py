...
def create_test_tinympc_settings(mode: str = "tinympc") -> TinyMPCSettings:
    """Create TinyMPC settings for testing.

    This implements Zac Manchester's TinyMPC algorithm:
    - ADMM-based QP solver for box-constrained control
    - Riccati recursion for unconstrained LQR solution
    - z-update projects controls onto bounds
    - y-update handles dual variables

    Args:
        mode: Control mode selection:
            "tinympc" - True TinyMPC: linearize at reference, fresh Riccati gains, ADMM
            "adaptive" - Adaptive TinyMPC: linearize at current state (better far from ref)
            "tvlqr" - Constrained TVLQR: use ALTRO's K gains with saturation (fast, not MPC)
    """
    # Mode settings
    if mode == "tvlqr":
        ...
        use_altro_gains = False
        use_true_mpc = True
    else:  # "tinympc" (default)
        use_altro_gains = False
        use_true_mpc = False
    ...
    return TinyMPCSettings(
        ...
        check_interval=5,
        # MPC horizon
        track_horizon=10,    # 10-step horizon as in TinyMPC paper
        track_dt=1.0,
        ...
        # Control mode (set based on mode parameter)
        use_altro_gains=use_altro_gains,
        use_true_mpc=use_true_mpc,
        # Debug
        verbose=1,
    )
...
def run_simulation(
    ...
    disturbance_torque: NDArray = None
) -> dict:
    """
    ...
    Args:
        disturbance_torque: Optional constant disturbance torque [Nm] to add.
            This simulates an unmodeled disturbance during tracking.
    ...
    for step in range(n_steps):
        ...
        # If disturbance torque is specified, create wrapper that adds it
        if disturbance_torque is not None:
            def dynamics_with_disturbance(t, x, u, os0, os1):
                xdot = sat.dynamics_for_solver(t, x, u, os0, os1)
                # Add disturbance torque to angular acceleration (w_dot)
                # w_dot = J^{-1} @ (torque - w x Jw)
                # Disturbance adds: J^{-1} @ disturbance_torque
                J_inv = sat.invJ_noRW
                xdot[0:3] += J_inv @ disturbance_torque
                return xdot
            dynamics_fn = dynamics_with_disturbance
        else:
            dynamics_fn = sat.dynamics_for_solver
...
def test_tinympc_python(sat, orbit, goals, x_0, t_start_cent, duration_sec, planner_settings, tinympc_settings,
                        tight_bounds_factor=None, disturbance_torque=None, mpc_knows_disturbance=True):
    """Test the pure Python TinyMPC controller.

    Args:
        mpc_knows_disturbance: If True and disturbance_torque is set, MPC will
            include the disturbance in its prediction model, allowing it to
            anticipate and compensate proactively (the key MPC advantage).
    """
    ...
    # Apply tight bounds if specified
    if tight_bounds_factor is not None:
        original_bounds = (controller._mpc.u_min.copy(), controller._mpc.u_max.copy())
        controller._mpc.u_min *= tight_bounds_factor
        controller._mpc.u_max *= tight_bounds_factor
        print(f"  Applied tight bounds: factor={tight_bounds_factor}")
        print(f"  Original bounds: [{original_bounds[0][0]:.3f}, {original_bounds[1][0]:.3f}]")
        print(f"  New bounds: [{controller._mpc.u_min[0]:.3f}, {controller._mpc.u_max[0]:.3f}]")

    # Tell MPC about the disturbance so it can predict and compensate
    if disturbance_torque is not None and mpc_knows_disturbance:
        controller.set_known_disturbance(disturbance_torque)
        print(f"  MPC knows about disturbance: {disturbance_torque} N·m")
...
def test_tvlqr_baseline(sat, orbit, goals, x_0, t_start_cent, duration_sec, planner_settings, disturbance_torque=None):
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='tinympc',
                        choices=['tinympc', 'adaptive', 'tvlqr', 'all'],
                        help='TinyMPC mode: tinympc (default), adaptive (linearize at current), '
                             'tvlqr (use ALTRO gains), or all (test all modes)')
    parser.add_argument('--tight-bounds', type=float, default=None,
                        help='Tighten control bounds by factor (e.g., 0.5 for half). '
                             'Test if TinyMPC outperforms TVLQR when constraints are active.')
    parser.add_argument('--disturbance', type=float, default=None,
                        help='Add constant disturbance torque magnitude [N·m]. '
                             'Tests MPC advantage with predictable disturbance.')
    parser.add_argument('--mpc-blind', action='store_true',
                        help="Don't tell MPC about the disturbance. Used to compare "
                             "MPC-with-knowledge vs MPC-blind vs TVLQR.")
    ...
    # Create disturbance torque if specified
    disturbance_torque = None
    if args.disturbance is not None:
        # Apply disturbance along a diagonal direction for worst-case
        disturbance_torque = args.disturbance * normalize(np.array([1.0, 1.0, 1.0]))
        print(f"\nDisturbance torque: {disturbance_torque} N·m (magnitude={args.disturbance:.4f})")

    # Collect results
    all_results = []

    # Determine which MPC modes to test
    if args.mode == 'all':
        # Test all modes
        mpc_modes = [
            ("tinympc", "TinyMPC (ADMM)"),
            ("adaptive", "TinyMPC (Adaptive)"),
            ("tvlqr", "Constrained TVLQR"),
        ]
    else:
        mode_names = {
            "tinympc": "TinyMPC (ADMM)",
        ...
        mpc_modes = [(args.mode, mode_names.get(args.mode, args.mode))]

    # Test Python TinyMPC in each mode
    mpc_knows = not args.mpc_blind
    for mode, mode_name in mpc_modes:
        tinympc_settings = create_test_tinympc_settings(mode=mode)
        results_py = test_tinympc_python(
            sat, orbit, goals, x_0, t_start_cent, duration_sec,
            planner_settings, tinympc_settings,
            tight_bounds_factor=args.tight_bounds,
            disturbance_torque=disturbance_torque,
            mpc_knows_disturbance=mpc_knows
        )
        if results_py is not None:
            if args.tight_bounds is not None:
                mode_name += f" (bounds×{args.tight_bounds})"
            if args.disturbance is not None:
                if mpc_knows:
                    mode_name += f" (dist={args.disturbance:.1e}, knows)"
                else:
                    mode_name += f" (dist={args.disturbance:.1e}, blind)"
            results_py['name'] = mode_name  # Override name to reflect mode
    ...
    if args.compare:
        ...
        if results_tvlqr is not None:
            if args.disturbance is not None:
                results_tvlqr['name'] += f" (dist={args.disturbance:.1e})"
