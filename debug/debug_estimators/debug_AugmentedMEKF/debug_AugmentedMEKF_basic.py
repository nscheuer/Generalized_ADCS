"""Run a no-bias end-to-end simulation with the augmented MEKF."""
...
def main() -> None:
    ...
    satellite = ADCS.Satellite(
        ...
        sensors=real_sensors, disturbances=[ADCS.disturbances.GG_Disturbance()],
    )
    ...
    est_satellite = ADCS.EstimatedSatellite(
        ...
        sensors=est_sensors, disturbances=[ADCS.disturbances.GG_Disturbance()],
    )
    ...
    x_hat = ADCS.EstimatorState(
        # EKF/MEKF enter after coarse acquisition: 5 deg attitude error and
        # a small rate error, rather than the 150 deg identity-to-truth gap.
        w=[0.0012, 0.0008, -0.0018],
        q=[np.cos(np.deg2rad(72.5)), 0.0, np.sin(np.deg2rad(72.5)), 0.0],
        cov=block_diag(np.eye(3) * 0.01**2, np.eye(3) * 0.15**2,),
        int_cov=block_diag(np.eye(3) * 1.0e-16, np.eye(3) * 1.0e-8,),
    )
    ...
    ADCS.plot(results, ADCS.plots.AttitudePlot(sources=["real", "estimated"]),
              layout=(1, 1), title="Augmented MEKF Estimator: No Biases")
    ADCS.plot(
        results,
        ...
        ADCS.plots.IlluminationPlot(), layout=(2, 2),
        title="Augmented MEKF Estimator: Gyro + MTM + SunPair",
    )
    run = results.first()
